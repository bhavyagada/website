...
export const renderImage = (renderer, image, isSelected, toolbar, isCropping) => {
  ...
  if (image.depthMaskCanvas) {
    if (!image.depthMaskTexture) updateDepthMaskTexture(renderer, image);
    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_2D, image.depthMaskTexture);
...
export const updateDepthMaskTexture = (renderer, image) => {
  const { gl } = renderer;
  
  if (!image.depthMaskTexture) {
    image.depthMaskTexture = gl.createTexture();
  }
